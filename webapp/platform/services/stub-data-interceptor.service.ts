...
export class StubbedDataRouter {
    registeredEndpoints: StubbedRoute[] = [];
    ...
    constructor(private configuration: StubbedDataInterceptorConfiguration) {
        const prePageLoadErrorResponses = window[STUBBED_ERROR_RESPONSES];
        if (prePageLoadErrorResponses) {
            Object.values(prePageLoadErrorResponses).forEach((errorResponseObject) => {
        ...
        const prePageLoadExtraResponses = window[STUBBED_EXTRA_RESPONSES];
        if (prePageLoadExtraResponses) {
            Object.values(prePageLoadExtraResponses).forEach((extraResponseObject) => {
    ...
    private addRoute(route: StubbedRoute) {
        ...
        if (enabled === true || (
            ...
            this.registeredEndpoints.push(route);
            ...
            this.registeredEndpoints.sort((a, b) => ((a.sortKey < b.sortKey) ? -1 : ((a.sortKey > b.sortKey) ? 1 : 0)));
    ...
    parse(request: HttpRequest<any>): StubbedMatchedRoute {
        try {
            ...
            if (path) {
                ...
                const stubbedErrorResponse = this.stubbedErrorResponses.get(`${method}-${path}`);
                if (stubbedErrorResponse) {
                    this.stubbedErrorResponses.delete(`${method}-${path}`);
                    throw stubbedErrorResponse;
                }

                // searching for a stubbed extra responses
                const stubbedExtraResponse = this.stubbedExtraResponses.get(`${method}-${path}`);
                if (stubbedExtraResponse) {
                    this.stubbedExtraResponses.delete(`${method}-${path}`);
                    return {
                        response: stubbedExtraResponse,
                        route: { method: method as StubbedMethod, path, handler: () => null }
                    };
                }

                // searching for a registered route
                this.registeredEndpoints.forEach((route) => {
                    if (!response) {
